...
NOTE: RHEL 9.4부터는 NVMe/TCP 시간 제한에 대한 기본 설정이 `ctrl_loss_tmo` 꺼집니다. 다시 시도 횟수에 제한이 없음을 의미합니다(무제한 재시도). 따라서 또는 `nvme connect-all` 명령(option-l)을 사용할 때 특정 시간 초과 기간을 `nvme connect` 수동으로 구성할 필요가 `ctrl_loss_tmo` 없습니다. 이 기본 동작에서는 경로 장애 발생 시 NVMe/TCP 컨트롤러가 시간 초과를 경험하지 않고 무기한 연결된 상태를 유지합니다.
