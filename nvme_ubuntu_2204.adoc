...
--
.단계
. Ubuntu 22.04 GA 커널에 포함된 기본 받은 편지함 qla2xxx 드라이버에는 ONTAP 지원에 필요한 최신 업스트림 수정 사항이 있습니다. 지원되는 어댑터 드라이버 및 펌웨어 버전을 실행하고 있는지 확인합니다.
+
[listing]
----
...
ONTAP 릴리스가 포함된 Ubuntu 22.04의 NVMe-oF 호스트 구성에 대해 알려진 문제는 없습니다.
