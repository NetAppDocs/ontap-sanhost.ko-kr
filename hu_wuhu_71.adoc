...
Host Utilities는 다양한 Windows 호스트 구성, 프로토콜 및 다중 경로 옵션을 지원합니다. 자세한 내용은 을 참조하십시오 https://["NetApp 상호 운용성 매트릭스 툴"^].
...
. 케이블 연결을 확인합니다
+
를 참조하십시오 https://["SAN 구성 가이드 를 참조하십시오"^] ONTAP 또는 용 https://["E-Series 하드웨어 케이블 연결 가이드 를 참조하십시오"^] 자세한 케이블 연결 및 구성 정보를 참조하십시오.
...
. ONTAP의 경우 WWPN으로 스위치를 조닝(Zoning)하십시오. 스토리지 컨트롤러의 물리적 포트가 아니라 LIF의 WWPN을 사용해야 합니다. 자세한 내용은 을 참조하십시오 https://["SAN 구성 가이드 를 참조하십시오"^].
...
|===
.관련 정보
을 참조하십시오 https://["Microsoft 문서"^] 레지스트리 매개 변수에 대한 자세한 내용은 를 참조하십시오.
...
.관련 정보
을 참조하십시오 https://["에뮬렉스"^] 또는 https://["QLogic"^] Site(사이트) 를 클릭하여 시간 초과 매개 변수에 대해 자세히 알아보십시오.
...
* 의 문제 해결 정보를 검토해야 합니다 https://["SAN 관리 가이드를 참조하십시오"^] ONTAP 버전용.
* 검색해야 합니다 https://["온라인 버그"^] 최근에 발견된 문제에 대해.
