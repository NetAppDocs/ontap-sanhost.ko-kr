...
NOTE: 단일 LUN에는 경로가 4개 이상이면 안 됩니다. 8개 이상의 경로로 인해 스토리지 장애가 발생했을 때 경로 문제가 발생할 수 있습니다.
...
NOTE: Oracle Linux 8.9 RedHat Enterprise Kernel을 구성하려면 link:hu_rhel_88.html#recommended-settings["권장 설정"]RHEL(Red Hat Enterprise Linux) 8.9용 을 사용하십시오.
