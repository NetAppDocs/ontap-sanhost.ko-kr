...
---
= Oracle Linux 7.9 및 NetApp ONTAP 사용
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
'multipath -ll' 명령을 사용하여 ONTAP LUN의 설정을 확인할 수 있습니다. 다음 섹션에서는 비 ASA 및 ASA 대상에 매핑된 LUN에 대한 샘플 다중 경로 출력을 제공합니다.
...
Oracle Linux 7.9 OS는 ONTAP LUN을 인식하여 모든 구성 매개 변수를 올바르게 설정하도록 컴파일되었습니다. 다중경로 데몬을 시작하려면 'multipath.conf' 파일이 있어야 하지만 'touch/etc/multipath.conf' 명령을 사용하여 빈 0바이트 파일을 생성할 수 있습니다
...
NOTE: Oracle Linux 7.9 RedHat Enterprise Kernel(RHCK)을 구성하려면 를 사용하십시오 link:hu_rhel_79.html#recommended-settings["권장 설정"] RHEL(Red Hat Enterprise Linux) 7.9의 경우
...
| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=1328944["1328943"^] | Emulex LPe32002(32G) 호스트를 사용하여 스토리지 페일오버 중에 I/O 지연이 관찰되었습니다 | QLogic LPe32002 32G HBA를 사용하는 Oracle Linux 7.8 커널(5.4.17-2011.1.2.el8uek.x86_64)에서 스토리지 페일오버 작업 중에 기존 활성 경로를 통해 입출력 작업이 계속되지 않을 수 있습니다. 스토리지 페일오버 중 기존 활성 경로를 계속 진행할 때 장애로 인해 I/O 진행이 중지되면 I/O 작업이 재개되지 않아 I/O 지연이 발생할 수 있습니다. I/O 작업은 스토리지 페일오버 반환 작업이 완료된 후에만 재개됩니다. | link:https://bugzilla.oracle.com/bugzilla/show_bug.cgi?id=17319["17319"^] 
|===