...
Oracle Linux 7.9 OS는 ONTAP LUN을 인식하여 모든 구성 매개 변수를 올바르게 설정합니다.  `multipath.conf`다중 경로 데몬을 시작하려면 파일이 있어야 합니다. 이 파일이 없는 경우 명령을 사용하여 빈 0바이트 파일을 만들 수 있습니다 `touch /etc/multipath.conf`
...
NOTE: Oracle Linux 7.9 RHCK(link:hu_rhel_79.html#recommended-settings["권장 설정"]Red Hat Enterprise Kernel)을 구성하려면 RHEL(Red Hat Enterprise Linux) 7.9용 을 사용하십시오.



== 알려진 문제

Oracle Linux 7.9 및 ONTAP 릴리스에는 다음과 같은 알려진 문제가 있습니다.
...
| 1440718 | SCSI 재검색을 수행하지 않고 LUN을 매핑 해제하거나 매핑하면 호스트의 데이터가 손상될 수 있습니다 | 를 설정할 때 `disable_changed_wwids` Multipath configuration parameter to Yes, WWID(Worldwide Identifier)가 변경되면 경로 디바이스에 대한 액세스가 비활성화됩니다. MultiPath는 경로의 WWID가 다중 경로 디바이스의 WWID로 복구될 때까지 경로 디바이스에 대한 액세스를 비활성화합니다. 를 참조하십시오 link:https://kb.netapp.com/Advice_and_Troubleshooting/Flash_Storage/AFF_Series/The_filesystem_corruption_on_iSCSI_LUN_on_the_Oracle_Linux_7["NetApp 기술 자료: Oracle Linux 7의 iSCSI LUN에서 파일 시스템이 손상되었습니다"^] 를 참조하십시오. 
|===