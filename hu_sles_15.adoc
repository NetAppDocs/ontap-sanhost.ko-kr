...
summary: ONTAP와 함께 SUSE Linux Enterprise Server 15를 사용하는 방법에 대해 설명합니다 
---
= NetApp ONTAP와 함께 SUSE Linux Enterprise Server 15를 사용합니다
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
SUSE Linux Enterprise Server 15의 경우 /etc/multipath.conf 파일이 있어야 하지만 파일을 구체적으로 변경할 필요는 없습니다. SUSE Linux Enterprise Server 15는 ONTAP LUN을 인식하고 올바르게 관리하는 데 필요한 모든 설정으로 컴파일됩니다.
...
SUSE Linux Enterprise Server 15 OS는 컴파일되어 ONTAP LUN을 인식하여 모든 구성 매개 변수를 올바르게 설정합니다. 다중경로 데몬을 시작하려면 'multipath.conf' 파일이 있어야 하지만 'touch/etc/multipath.conf' 명령을 사용하여 빈 0바이트 파일을 생성할 수 있습니다
...
| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=1154309["1154309"^] | 매핑된 LUN이 20개 이상인 SLES 15 호스트는 재부팅 후 유지보수 모드로 전환될 수 있습니다 | 매핑된 LUN이 20개 이상인 SLES 15 호스트는 재부팅 후 유지보수 모드로 전환될 수 있습니다. 유지보수 모드는 '유지 관리를 위해 루트 암호를 제공(또는 계속하려면 Control-D를 누름)' 메시지에 따라 단일 사용자 모드가 됩니다 | link:https://bugzilla.suse.com/show_bug.cgi?id=1104173["1104173"^] 
|===
