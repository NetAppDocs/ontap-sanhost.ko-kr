...
SUSE Linux Enterprise Server 15 OS는 컴파일되어 ONTAP LUN을 인식하여 모든 구성 매개 변수를 올바르게 설정합니다.  `multipath.conf`다중 경로 데몬을 시작하려면 파일이 있어야 합니다. 이 파일이 없는 경우 명령을 사용하여 빈 0바이트 파일을 만들 수 있습니다 `touch /etc/multipath.conf`
...
ONTAP가 포함된 SUSE Linux Enterprise Server 15 릴리스에는 다음과 같은 알려진 문제가 있습니다.
...
| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=1154309["1154309"^] | 매핑된 LUN이 20개 이상인 SLES 15 호스트는 재부팅 후 유지보수 모드로 전환될 수 있습니다 | 매핑된 LUN이 20개 이상인 SLES 15 호스트는 재부팅 후 유지보수 모드로 전환될 수 있습니다. 유지보수 모드는 '유지 관리를 위해 루트 암호를 제공(또는 계속하려면 Control-D를 누름)' 메시지에 따라 단일 사용자 모드가 됩니다 
|===