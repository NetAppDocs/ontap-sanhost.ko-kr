...
summary: ONTAP에서 Ubuntu 24.04를 사용하는 방법을 설명합니다 
---
= ONTAP에서 Ubuntu 24.04를 사용합니다
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
[role="lead"]
ONTAP SAN 호스트 구성 설정을 사용하여 ONTAP를 대상으로 Ubuntu 24.04를 구성할 수 있습니다.


NOTE: NetApp Linux 통합 호스트 유틸리티 소프트웨어 패키지는 Ubuntu 24.04 OS에서 사용할 수 없습니다.
...
Ubuntu 24.04의 경우 `/etc/multipath.conf` 파일이 존재해야 하지만 ONTAP LUN을 인식하고 올바르게 관리하는 데 필요한 설정으로 Ubuntu 24.04가 컴파일되므로 파일을 변경할 필요가 없습니다.
...
Ubuntu 24.04 OS는 ONTAP LUN을 인식하도록 컴파일되고 ASA 및 비 ASA 구성에 대해 모든 구성 매개 변수를 자동으로 올바르게 설정합니다. 또한 다음과 같은 권장 설정을 사용하여 호스트 구성의 성능을 더욱 최적화할 수 있습니다.
...
| 공급업체 | 넷엡 
|===
다음 예제에서는 재정의된 기본값을 수정하는 방법을 보여 줍니다. 이 경우 `multipath.conf` 파일에서 및 `no_path_retry` ONTAP LUN과 호환되지 않는 값을 `path_checker` 정의합니다. 다른 SAN 스토리지가 아직 호스트에 연결되어 있으므로 제거할 수 없는 경우 이러한 매개 변수를 특히 디바이스 스탠자가 있는 ONTAP LUN에 대해 수정할 수 있습니다.
...
ONTAP 릴리스가 포함된 Ubuntu 24.04에는 알려진 문제가 없습니다.
