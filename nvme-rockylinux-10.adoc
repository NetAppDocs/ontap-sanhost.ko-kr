...
summary: ONTAP 스토리지를 사용하여 Rocky Linux 10에 NVMe-oF 호스트를 구성하는 방법 
---
= ONTAP 스토리지를 사용하여 NVMe-oF에 맞게 Rocky Linux 10 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
<stdin>에 해결되지 않은 지시어가 있습니다 - include::_include/nvme/reuse-nvme-introduction.adoc[]

.이 작업에 대해
Rocky Linux 10의 NVMe-oF 호스트 구성에서는 다음 지원과 기능을 사용할 수 있습니다. 또한 구성 프로세스를 시작하기 전에 알려진 제한 사항을 검토해야 합니다.
...
** ONTAP 9.12.1부터 NVMe/TCP에 대한 보안 인밴드 인증이 지원됩니다. Rocky Linux 10에서는 NVMe/TCP에 대한 안전한 인밴드 인증을 사용할 수 있습니다.
...
* 사용 가능한 기능:
+
** Rocky Linux 10부터 기본 NVMe 멀티패스가 항상 활성화되어 있으며, NVMe-oF에 대한 DM 멀티패스 지원은 지원되지 않습니다.
...
다음 절차에 따라 지원되는 최소 Rocky Linux 10 소프트웨어 버전을 확인하세요.

.단계
. 서버에 Rocky Linux 10을 설치하세요. 설치가 완료되면 지정된 Rocky Linux 10 커널이 실행 중인지 확인하세요.
+
[source, cli]
----
uname -r
...
<stdin>에 해결되지 않은 지시어가 있습니다 - include::_include/nvme/reuse_nvme_enabling_broadcom_1mb_size.adoc[]



== 5단계: NVMe 부팅 서비스 확인

Rocky Linux 10에서는  `nvmefc-boot-connections.service` 그리고  `nvmf-autoconnect.service` NVMe/FC에 포함된 부팅 서비스  `nvme-cli` 패키지는 시스템이 부팅될 때 자동으로 활성화됩니다.
...
<stdin>에 해결되지 않은 지시어가 있습니다 - include::_include/nvme/reuse-nvme-configure-nvme-tcp.adoc[]
...
<stdin>에 해결되지 않은 지시어 - include::_include/nvme/reuse-nvme-validate-nvme-of.adoc[]
...
ONTAP 9.12.1부터 Rocky Linux 10 호스트와 ONTAP 컨트롤러 간의 NVMe/TCP를 통한 안전한 인밴드 인증이 지원됩니다.
...
cat /etc/nvme/hostnqn
----
. Rocky Linux 10 호스트에 대한 dhchap 키를 생성합니다.
+
