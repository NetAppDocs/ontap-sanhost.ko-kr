...
summary: ONTAP 스토리지를 사용하여 Rocky Linux 10.0에 NVMe-oF 호스트를 구성하는 방법 
---
= ONTAP 스토리지를 사용하여 NVMe-oF에 Rocky Linux 10.0 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Rocky Linux 10.0에 대해 NVMe over Fabrics(NVMe-oF) 호스트를 구성하는 방법을 알아보세요. 추가 지원 및 기능 정보는 다음을 참조하세요. link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP 지원 및 기능"].

*Rocky Linux 10.0을 사용하는 NVMe-oF에는 다음과 같은 알려진 제한 사항이 있습니다.*

* NVMe-oF에 대한 DM 멀티패스 지원은 지원되지 않습니다.
...
.단계
. 서버에 Rocky Linux 10.0을 설치합니다.  설치가 완료되면 필요한 Rocky Linux 10.0 커널을 실행하고 있는지 확인하세요.
+
[source, cli]
----
uname -r
...
ONTAP 9.12.1부터 Rocky Linux 10.0 호스트와 ONTAP 컨트롤러 간의 NVMe/TCP를 통한 안전한 인밴드 인증이 지원됩니다.
...
cat /etc/nvme/hostnqn
----
. Rocky Linux 10.0 호스트에 대한 dhchap 키를 생성합니다.
+
