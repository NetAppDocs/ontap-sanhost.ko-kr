...
Rocky Linux 10을 사용하면  `nvmefc-boot-connections.service` 그리고  `nvmf-autoconnect.service` NVMe/FC에 포함된 부팅 서비스  `nvme-cli` 패키지는 시스템이 부팅될 때 자동으로 활성화됩니다. 부팅이 완료되면 서비스가 활성화되었는지 확인하세요.
...
NOTE: Rocky Linux 9.5부터 NVMe/TCP의 기본 설정은  `ctrl_loss_tmo` 시간 초과가 해제되어 있습니다. 다시 시도 횟수에 제한이 없음을 의미합니다(무제한 재시도). 따라서 또는 `nvme connect-all` 명령(option-l)을 사용할 때 특정 시간 초과 기간을 `nvme connect` 수동으로 구성할 필요가 `ctrl_loss_tmo` 없습니다. 이 기본 동작에서는 경로 장애 발생 시 NVMe/TCP 컨트롤러가 시간 초과를 경험하지 않고 무기한 연결된 상태를 유지합니다.
