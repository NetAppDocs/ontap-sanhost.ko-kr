...
---
= NetApp ONTAP와 함께 SUSE Linux Enterprise Server 15 SP4를 사용합니다
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
'<DevID>'을(를) 제외할 장치의 'WWID' 문자열로 바꿉니다. 다음 명령어를 이용하여 WWID를 결정한다.
...
----
. 이 WWID를 '/etc/multipath.conf'의 블랙 리스트 스탠자에 추가하십시오.
+
[listing]
----
blacklist {
...
아래 표에는 ONTAP LUN에 대한 중요 다중 경로 매개 변수와 필요한 값이 나와 있습니다. 호스트가 다른 공급업체의 LUN에 접속되어 있고 이러한 매개 변수 중 하나라도 재정의될 경우 ONTAP LUN에 특별히 적용되는 'multipath.conf'에서 나중에 stanzas로 수정해야 합니다. 그렇지 않으면 ONTAP LUN이 예상대로 작동하지 않을 수 있습니다. 이러한 기본값은 NetApp 및/또는 OS 공급업체에 문의하여 영향을 완전히 이해해야 합니다.
...
SUSE Linux Enterprise Server 15 SP4에는 알려진 문제가 없습니다.
