...
아래 표에는 ONTAP LUN에 대한 중요 다중 경로 매개 변수와 필요한 값이 나와 있습니다. 호스트가 다른 공급업체의 LUN에 접속되어 있고 이러한 매개 변수가 무시되는 경우 나중에 에서 stanzas를 사용하여 수정해야 합니다 `multipath.conf` 이는 특히 ONTAP LUN에 적용됩니다. 그렇지 않으면 ONTAP LUN이 예상대로 작동하지 않을 수 있습니다. 이러한 기본값은 NetApp 및/또는 OS 공급업체에 문의하여 영향을 완전히 이해해야 합니다.
