...
Oracle Linux 7.7 OS는 ONTAP LUN을 인식하여 모든 구성 매개 변수를 올바르게 설정합니다.  `multipath.conf`다중 경로 데몬을 시작하려면 파일이 있어야 합니다. 이 파일이 없으면 다음 명령을 사용하여 빈 0바이트 파일을 만들 수 있습니다.
...
NOTE: Oracle Linux 7.7 RHCK(Red Hat Enterprise Kernel)을 구성하려면 link:hu_rhel_77.html#recommended-settings["권장 설정"]RHEL(Red Hat Enterprise Linux) 7.7용 을 사용하십시오.



== 알려진 문제

ONTAP 릴리스가 포함된 Oracle Linux 7.7에는 다음과 같은 알려진 문제가 있습니다.
