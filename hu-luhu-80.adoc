...
[role="lead"]
Linux 호스트 유틸리티는 Linux 호스트에 연결된 ONTAP 스토리지를 관리하는 데 도움이 됩니다. NetApp은 Linux 호스트 유틸리티 설치를 강력히 권장하지만 필수 사항은 아닙니다. Linux 호스트 유틸리티는 관리 효율성을 높이고 NetApp 고객 지원 팀이 구성에 대한 정보를 수집하는 데 도움을 줍니다.
...
.이 작업에 대해
Linux 호스트 유틸리티를 설치해도 호스트의 설정이나 동작은 변경되지 않습니다.
...
NOTE: Linux Host Utilities는 libHBAAPI.so 라이브러리에 대한 설치 종속성이 있습니다. 호스트가 iSCSI 전용 구성을 위한 것이더라도 위에 나열된 패키지 중 하나를 설치해야 합니다. 또는 LUN 및 경로 관리를 위해 OS 수준의 도구를 사용할 수도 있습니다.
