...
[role="lead"]
NVMe/FC 및 기타 전송을 포함한 NVMe over Fabrics(NVMe-oF)는 ANA(비대칭 네임스페이스 액세스)가 포함된 Oracle Linux 9.0에서 지원됩니다. NVMe-oF 환경의 경우, ANA는 iSCSI 및 FC 환경에서 ALUA 다중 경로와 동일하며 커널 내 NVMe 다중 경로를 통해 구현됩니다.
...
.단계
. 서버에 Oracle Linux 9.0 GA를 설치합니다. 설치가 완료되면 지정된 Oracle Linux 9.0 GA 커널을 실행 중인지 확인합니다.
+
[listing]
----
# uname -r
...
.단계
. Oracle Linux 9.0 호스트에서 다음 NVMe/FC 설정을 확인합니다.
+
[listing]
----
...
구성에 문제가 있는 경우 자세한 정보 로깅은 문제 해결에 중요한 정보를 제공할 수 있습니다.
