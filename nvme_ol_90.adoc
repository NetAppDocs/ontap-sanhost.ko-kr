...
--
OL 9.0 GA 커널에 포함된 기본 받은 편지함 qla2xxx 드라이버에는 최신 업스트림 픽스가 있습니다. 이러한 수정 사항은 ONTAP 지원에 필수적입니다.
