...
---
= Citrix XenServer를 NetApp ONTAP와 함께 사용하여 Hyper-V를 사용하지 않으며
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
이 문서에서는 FC, FCoE 및 iSCSI 프로토콜을 지원하는 Citrix XenServer 7 시리즈 OS 릴리스에 대한 ONTAP SAN 호스트 구성 설정에 대해 설명합니다.
...
----
다중 경로를 통해 관리하지 않으려는 디바이스가 있거나 기본값을 재정의하는 기존 설정이 있는 경우를 제외하고 "/etc/multipath.conf" 파일에 콘텐츠를 추가할 필요는 없습니다. 다음 구문을 'multitpath.conf' 파일에 추가하여 원치 않는 디바이스를 제외할 수 있습니다.
...
이 Citrix XenServer 7.x의 경우 `sda` 블랙리스트에 추가해야 하는 로컬 SCSI 디스크입니다.
