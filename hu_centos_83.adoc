...
summary: ONTAP에서 CentOS 8.3을 사용하는 방법에 대해 설명합니다 
---
= CentOS 8.3과 NetApp ONTAP 사용
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
CentOS 8.3의 경우 /etc/multipath.conf 파일이 있어야 하지만 파일을 구체적으로 변경할 필요는 없습니다. CentOS 8.3은 ONTAP LUN을 인식하고 올바르게 관리하는 데 필요한 모든 설정으로 컴파일됩니다.
...
CentOS 8.3 OS는 ONTAP LUN을 인식하여 ASA 및 비 ASA 구성 모두에 대해 모든 구성 매개 변수를 올바르게 자동 설정하도록 컴파일됩니다. 다중경로 데몬을 시작하려면 'multipath.conf' 파일이 있어야 하지만 'touch/etc/multipath.conf' 명령을 사용하여 빈 0바이트 파일을 생성할 수 있습니다
...
CentOS(Red Hat 호환 커널) 알려진 문제는 를 참조하십시오 link:hu_rhel_83.html#known-problems-and-limitations["알려진 문제"] Red Hat Enterprise Linux(RHEL) 8.3의 경우
