...
SUSE Linux Enterprise Server 12 SP1 OS는 컴파일되어 ONTAP LUN을 인식하고 모든 구성 매개 변수를 올바르게 자동 설정합니다.  `multipath.conf`다중 경로 데몬을 시작하려면 파일이 있어야 합니다. 이 파일이 없으면 다음 명령을 사용하여 빈 0바이트 파일을 만들 수 있습니다.
