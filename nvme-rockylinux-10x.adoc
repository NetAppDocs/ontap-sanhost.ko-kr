...
summary: ONTAP 스토리지를 사용하여 Rocky Linux 10.x용 NVMe-oF 호스트를 구성하는 방법 
---
= ONTAP 스토리지를 사용하는 NVMe-oF용 Rocky Linux 10.x 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Rocky Linux 10.x용 NVMe-oF(NVMe over Fabrics) 호스트를 구성하는 방법을 알아보세요. 추가 지원 및 기능 정보는 다음을 참조하세요. link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP 지원 및 기능"].

Rocky Linux 10.x에서 NVMe-oF를 사용할 때 다음과 같은 알려진 제한 사항이 있습니다.
...
.단계
. 서버에 Rocky Linux 10.x를 설치하세요.  설치가 완료되면 필요한 Rocky Linux 10.x 커널이 실행 중인지 확인하십시오.
+
[source, cli]
----
uname -r
...
Rocky Linux 10.x 호스트와 ONTAP 컨트롤러 간에 NVMe/TCP를 통한 안전한 대역 내 인증이 지원됩니다.
...
cat /etc/nvme/hostnqn
----
. Rocky Linux 10.x 호스트용 dhchap 키를 생성합니다.
+
