...
** NetApp은 Oracle에서 전체 구성을 지원하고 LUN에 직접 액세스하는 모든 파티션이 지원되는 구성의 link:https://imt.netapp.com/matrix/#welcome["IMT"^]에 나열된 경우 Solaris 가상화 기술의 사용을 지원합니다. 여기에는 루트 컨테이너, LDOM I/O 도메인 및 NPIV를 사용하여 LUN에 액세스하는 LDOM이 포함됩니다.
...
+
.. Solaris 호스트에 최신 Support Repository Update(SRU)가 설치되어 있는지 확인하십시오.
+
[source, cli]
----
pkg info entire
