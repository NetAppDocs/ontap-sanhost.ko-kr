...
---
= NetApp ONTAP에서 IBM AIX 7.2 및/또는 PowerVM(VIOS 3.1) 사용
:toc: macro
:hardbreaks:
:toclevels: 1
:toc: 
...
== AIX/VIOS 호스트 유틸리티 설치

AIX MPIO를 NetApp ONTAP 스토리지와 함께 사용하는 동안 AIX 호스트 유틸리티 키트를 설치해야 합니다.

NetApp Support 사이트에서 Host Utilities 소프트웨어 패키지가 포함된 압축 파일을 다운로드할 수 있습니다. 파일이 있는 경우 Host Utilities를 설치하는 데 필요한 두 개의 소프트웨어 패키지를 가져오려면 파일의 압축을 풀어야 합니다.
...
다음은 NetApp ONTAP LUN의 몇 가지 권장 매개 변수 설정입니다. ONTAP LUN에 대한 중요 매개 변수는 NetApp 호스트 유틸리티 키트를 설치한 후 자동으로 설정됩니다.
