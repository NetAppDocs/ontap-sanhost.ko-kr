...
summary: ONTAP에서 Oracle Linux 9.3을 사용하는 방법에 대해 설명합니다 
---
= ONTAP에서 Oracle Linux 9.3을 사용합니다
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
ONTAP SAN 호스트 구성 설정을 사용하여 ONTAP를 대상으로 Oracle Linux 9.3을 구성할 수 있습니다.
...
OL 9.3의 경우 `/etc/multipath.conf` 파일이 존재해야 하지만 파일을 구체적으로 변경할 필요는 없습니다. OL 9.3는 ONTAP LUN을 인식하고 올바르게 관리하는 데 필요한 모든 설정으로 컴파일됩니다.
...
Oracle Linux 9.3 OS는 ONTAP LUN을 인식하고 ASA 및 비 ASA 구성 모두에 대해 모든 구성 매개 변수를 올바르게 자동으로 설정합니다. 다음과 같은 권장 설정을 사용하여 호스트 구성의 성능을 더욱 최적화할 수 있습니다.
...
NOTE: Oracle Linux 9.3 RHCK(Red Hat Compatible Kernel)을 구성하려면 RHEL(Red Hat Enterprise Linux) 9.3용 을 link:hu_rhel_93.html#recommended-settings["권장 설정"]사용하십시오.
...
ONTAP 릴리즈가 포함된 Oracle Linux 9.3에는 다음과 같은 알려진 문제가 있습니다.
