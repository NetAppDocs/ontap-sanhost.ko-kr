...
----
====
. 처음으로 다중 경로를 구성할 때 권장 설정을 로드하기 위해 다중 경로 서비스를 활성화하고 시작해야 할 수 있습니다.
+
[source, cli]
----
...
호스트가 다른 벤더의 LUN에 연결되어 있고 다중 경로 파라미터 설정이 재정의된 경우, `/etc/multipath/conf.d/custom.conf` 파일에서 ONTAP LUN에 특별히 적용되는 스탠자를 수정하여 설정을 바로잡아야 합니다. 파일에서 아래로 스크롤하여 이러한 스탠자를 추가하십시오. 파라미터를 수정하지 않으면 ONTAP LUN이 예상대로 작동하지 않을 수 있습니다.
