...
summary: ONTAP에서 Oracle Linux 9.4을 사용하는 방법에 대해 설명합니다 
---
= ONTAP에서 Oracle Linux 9.4을 사용합니다
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
ONTAP SAN 호스트 구성 설정을 사용하여 ONTAP를 대상으로 Oracle Linux 9.4을 구성할 수 있습니다.
...
.시작하기 전에
를 사용하여 link:https://mysupport.netapp.com/matrix/imt.jsp?components=91241;&solution=236&isHWU&src=IMT["NetApp 상호 운용성 매트릭스 툴"^]OS, HBA, HBA 펌웨어, HBA 부팅 BIOS 및 ONTAP 버전이 지원되는지 확인합니다.
...
Oracle Linux 9.4에 다중 경로를 사용하여 ONTAP LUN을 관리할 수 있습니다. 경로 다중화를 `/etc/multipath.conf` 사용하려면 파일이 있어야 합니다. 파일이 없는 경우 `/etc/multipath.conf` 터치 명령을 사용하여 빈 0바이트 파일을 만들 수 `/etc/multipath.conf` 있습니다. Oracle Linux 9.4는 ONTAP LUN을 인식하고 올바르게 관리하는 데 필요한 모든 설정으로 컴파일되므로 파일을 변경할 필요가 `/etc/multipath.conf` 없습니다.


NOTE: 단일 LUN에는 4개 이상의 경로가 필요하지 않습니다. 4개 이상의 경로로 인해 스토리지 장애 시 경로 문제가 발생할 수 있습니다.
...
Oracle Linux 9.4 OS는 ONTAP LUN을 인식하고 ASA 및 비 ASA 구성 모두에 대해 모든 구성 매개 변수를 올바르게 자동으로 설정합니다. 다음과 같은 권장 설정을 사용하여 호스트 구성의 성능을 더욱 최적화할 수 있습니다.
...
NOTE: Oracle Linux 9.4 RHEK(Red Hat Enterprise Kernel)를 구성하려면 RHEL(Red Hat Enterprise Linux) 9.4용 을 link:hu_rhel_94.html#recommended-settings["권장 설정"]사용하십시오.
...
ONTAP 릴리즈가 포함된 OL 9.4의 FC 호스트 구성에는 알려진 문제가 없습니다.
