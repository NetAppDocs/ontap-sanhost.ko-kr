...
AIX Host Utilities의 최신 릴리즈에는 다음과 같은 새로운 기능과 개선 사항이 포함되어 있습니다.
...
| 해당 없음 | NetApp ONTAP iSCSI LUN에 대한 SCSI UNMAP 지원 | AIX Host Utilities 7.0은 FC 디바이스에 대해 SCSI UNMAP 기능만 지원합니다. iSCSI 장치에 대한 SCSI UNMAP 지원은 이 릴리스에 포함되지 않습니다. 
