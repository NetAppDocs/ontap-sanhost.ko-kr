...
---
= ONTAP와 함께 Oracle Linux 8.4를 사용합니다
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
ONTAP SAN 호스트 구성 설정을 사용하여 ONTAP를 대상으로 Oracle Linux 8.4를 구성할 수 있습니다.
...
NOTE: Oracle Linux 8.4 RHCK(Red Hat Enterprise Kernel)를 구성하려면 RHEL(Red Hat Enterprise Linux) 8.4용 을 link:hu_rhel_84.html#recommended-settings["권장 설정"]사용하십시오.
...
Oracle Linux 8.4과 ONTAP 릴리스의 경우 알려진 문제가 없습니다.


NOTE: Oracle Linux(Red Hat 호환 커널) 알려진 문제는 해당 Red Hat Enterprise Linux 릴리스 설명서의 섹션을 참조하십시오link:hu_rhel_84.html#known-problems-and-limitations["알려진 문제"].
