...
SM-BC(SnapMirror Business Continuity) 환경에서 계획되지 않은 사이트 페일오버 스위치오버가 발생할 때 Solaris 클라이언트 애플리케이션이 중단 없이 작동하도록 하려면 Solaris 11.3 호스트에 다음 설정을 구성해야 합니다. 이 설정은 페일오버 모듈 – 을 재정의합니다 `f_tpgs` 모순을 감지하는 코드 경로가 실행되지 않도록 합니다.
