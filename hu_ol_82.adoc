...
NOTE: Oracle Linux 8.2 RHCK(Red Hat Enterprise Kernel)를 구성하려면 RHEL(Red Hat Enterprise Linux) 8.2용 을 link:hu_rhel_82.html#recommended-settings["권장 설정"]사용하십시오.



== 알려진 문제

Oracle Linux 8.2과 ONTAP 릴리스의 경우 알려진 문제가 없습니다.


NOTE: Oracle Linux(Red Hat 호환 커널) 알려진 문제는 RHEL(Red Hat Enterprise Linux) 8.2에 대한 를 참조하십시오link:hu_rhel_82.html#known-problems-and-limitations["알려진 문제"].
