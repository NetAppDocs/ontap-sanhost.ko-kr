...
summary: ONTAP 사용하여 Rocky Linux 8.x용 NVMe-oF 호스트를 구성하는 방법 
---
= ONTAP 스토리지를 사용하는 NVMe-oF용 Rocky Linux 8.x 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Rocky Linux 8.x용 NVMe-oF(NVMe over Fabrics) 호스트를 구성하는 방법을 알아보세요. 추가 지원 및 기능 정보는 다음을 참조하세요. link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP 지원 및 기능"].

Rocky Linux 8.x에서 NVMe-oF를 사용할 때 다음과 같은 알려진 제한 사항이 있습니다.

* NVMe-oF 프로토콜을 사용한 SAN 부팅은 현재 지원되지 않습니다.
* Rocky Linux 8.x에서는 NVMe-oF 호스트에서 커널 내 NVMe 멀티패스가 기본적으로 비활성화되어 있으므로 수동으로 활성화해야 합니다.
* 알려진 문제로 인해 NVMe/TCP는 기술 미리보기로 제공됩니다.




== 1단계: Rocky Linux 및 NVMe 소프트웨어를 설치하고 구성을 확인합니다.

NVMe-oF를 사용하도록 호스트를 구성하려면 호스트 및 NVMe 소프트웨어 패키지를 설치하고, 멀티패싱을 활성화하고, 호스트의 NQN 구성을 확인해야 합니다.

.단계
. 서버에 Rocky Linux 8.x를 설치하세요.  설치가 완료되면 필요한 Rocky Linux 8.x 커널이 실행 중인지 확인하십시오.
+
[source, cli]
----
uname -r
...
<stdin>에서 해결되지 않은 지시문 - include::_include/nvme/linux-validate-software-version-rockylinux.adoc[]
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Rocky Linux 8.x NVMe-oF 호스트에서 영구 검색 컨트롤러가 중복 생성됩니다. | NVMe-oF 호스트에서는 "nvme discover -p" 명령을 사용하여 영구 검색 컨트롤러(PDC)를 생성할 수 있습니다.  하지만 NVMe-oF 호스트에서 Rocky Linux 8.x를 실행하는 경우 "nvme discover -p" 명령을 실행할 때마다 중복된 PDC가 생성됩니다. 이 명령을 사용하면 개시자-대상 조합당 하나의 PDC만 생성되어야 합니다. 그러나 NVMe-oF 호스트에서 Rocky Linux 8.x를 실행하는 경우 "nvme discover -p"가 실행될 때마다 중복 PDC가 생성됩니다. 이로 인해 호스트와 대상 모두에서 불필요한 리소스 사용이 발생합니다. 
|===