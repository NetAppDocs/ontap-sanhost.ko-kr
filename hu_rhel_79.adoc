...
summary: ONTAP와 함께 Red Hat Enterprise Linux 7.9를 사용하는 방법을 설명합니다 
---
= NetApp ONTAP와 함께 Red Hat Enterprise Linux 7.9 사용
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
RHEL(Red Hat Enterprise Linux) 7.9의 경우 /etc/multipath.conf 파일이 존재해야 하지만 파일을 구체적으로 변경할 필요는 없습니다. RHEL 7.9는 ONTAP LUN을 인식하고 올바르게 관리하는 데 필요한 모든 설정으로 컴파일됩니다.
...
RHEL 7.9 OS는 ONTAP LUN을 인식하여 ASA 및 비 ASA 구성 모두에 대해 모든 구성 매개 변수를 올바르게 자동 설정하도록 컴파일됩니다.
