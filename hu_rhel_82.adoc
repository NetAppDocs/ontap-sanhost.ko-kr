...
summary: RHEL 8.2 호스트에 Linux Host Utilities를 설치하여 ONTAP LUN으로 FCP 및 iSCSI 프로토콜 작업을 관리합니다. 
---
= ONTAP 스토리지를 사용하여 FCP에 RHEL 8.2를 구성하고 iSCSI를 구성합니다
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux 호스트 유틸리티 소프트웨어는 ONTAP 스토리지에 연결된 Linux 호스트를 위한 관리 및 진단 툴을 제공합니다. Red Hat Enterprise Linux(RHEL) 8.2 호스트에 Linux 호스트 유틸리티를 설치할 때 Host Utilities를 사용하면 ONTAP LUN에서 FCP 및 iSCSI 프로토콜 작업을 관리할 수 있습니다.
...
RHEL 8.2에서 다중 경로를 사용하여 ONTAP LUN을 관리할 수 있습니다.
