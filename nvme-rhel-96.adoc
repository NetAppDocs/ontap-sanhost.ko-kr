...
---
= ONTAP 스토리지를 사용하여 NVMe-oF에 대한 RHEL 9.6 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Red Hat Enterprise Linux(RHEL) 9.6에 대해 NVMe over Fabrics(NVMe-oF) 호스트를 구성하는 방법을 알아보세요.  추가 지원 및 기능 정보는 다음을 참조하세요.link:hu-nvme-index.html["NVME-oF 개요"^] .

*RHEL 9.6을 사용하는 NVMe-oF에는 다음과 같은 알려진 제한 사항이 있습니다.*
...
.단계
. 서버에 RHEL 9.6을 설치합니다.  설치가 완료되면 필요한 RHEL 9.6 커널을 실행하고 있는지 확인하세요.
+
[source, cli]
----
uname -r
...
=== udev 규칙에서 iopolicy를 수정합니다.

RHEL 9.6은 NVMe-oF에 대한 기본 iopolicy를 다음과 같이 설정합니다. `round-robin` .  iopolicy를 변경하려면 `queue-depth` , udev 규칙 파일을 다음과 같이 수정합니다.
...
----
. 다음 명령을 실행하여 udev 규칙을 다시 로드하고 변경 사항을 적용합니다.
+
[source, cli]
----
...
----
. 하위 시스템의 현재 iopolicy를 확인하세요.  <하위 시스템>을 적절한 이름으로 바꾸세요. 예: `nvme-subsys0` .
+
[source, cli]
----
...
ONTAP 9.12.1부터 RHEL 10.0 호스트와 ONTAP 컨트롤러 간의 NVMe/TCP를 통한 안전한 인밴드 인증이 지원됩니다.
