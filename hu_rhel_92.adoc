...
summary: ONTAP에서 Red Hat Enterprise Linux 9.2를 사용하는 방법을 설명합니다 
---
= ONTAP와 함께 Red Hat Enterprise Linux 9.2를 사용하십시오
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
ONTAP SAN 호스트 구성 설정을 사용하여 ONTAP가 포함된 Red Hat Enterprise Linux 9.2를 대상으로 구성할 수 있습니다.
...
RHEL(Red Hat Enterprise Linux) 9.2의 경우 `/etc/multipath.conf` 파일이 있어야 하지만 파일을 구체적으로 변경할 필요는 없습니다. RHEL 9.2는 ONTAP LUN을 인식하고 올바르게 관리하는 데 필요한 모든 설정으로 컴파일됩니다.
...
RHEL 9.2 OS는 ONTAP LUN을 인식하여 ASA 및 비 ASA 구성 모두에 대해 모든 구성 매개 변수를 올바르게 자동 설정하도록 컴파일되었습니다. 다음과 같은 권장 설정을 사용하여 호스트 구성의 성능을 더욱 최적화할 수 있습니다.
...
ONTAP 릴리스의 RHEL 9.2에는 다음과 같은 알려진 문제가 있습니다.
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/1508554["1508554"^] | NetApp Linux Host Utilities CLI에서 Emulex HBA 어댑터 검색을 지원하려면 추가 라이브러리 패키지 종속성이 필요합니다 | RHEL 9.2에서 NetApp Linux SAN 호스트 유틸리티 CLI를 참조하십시오 `sanlun fcp show adapter -v` HBA 검색을 지원하는 라이브러리 패키지 종속성을 찾을 수 없기 때문에 실패합니다. | 해당 없음 


| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/1537359["1537359)를 참조하십시오"^] | Emulex HBA를 사용하는 Red Hat Linux 9.2 SAN 부팅 호스트에서는 커널이 중단되는 작업이 발생합니다 | 스토리지 페일오버 반환 작업 중에 Emulex 호스트 버스 어댑터(HBA)가 설치된 Red Hat Linux 9.2 SAN 부팅 호스트에서 커널이 중단되는 작업이 발생합니다. 커널이 중단되면 운영 체제가 재부팅되고 `kdump` 가 구성되어 있으면 가 생성됩니다 `vmcore` 파일 아래에 있습니다 `/var/crash/` 디렉토리. 이 문제는 와 함께 삼각 측량을 진행 중입니다 `lpfc` 드라이버를 재생하지만 일관되게 재현할 수는 없습니다. | link:https://bugzilla.redhat.com/show_bug.cgi?id=2173947["2173947"^] 
|===