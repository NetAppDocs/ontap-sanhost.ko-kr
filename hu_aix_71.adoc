...
NOTE: AIX에서는 MPIO가 활성화될 때까지 SAN 부팅 디스크에 하나의 경로만 표시되어야 합니다.  AIX 부팅 디스크를 설치하거나 마이그레이션할 때는 일반적으로 호스트의 FC 영역 설정을 일시적으로 제한하여 FC 이니시에이터 하나와 FC 타겟 하나만 포함하도록 해야 합니다.  호스트 유틸리티(PCM 패키지 포함)를 설치한 후 운영 체제를 재부팅하여 MPIO를 완전히 활성화할 수 있습니다.  그런 다음 추가 FC 이니시에이터와 FC 타겟을 영역에 추가합니다.  그런 다음 `cfgmgr -v` 및 `bosboot -a`를 실행하여 사용 가능한 모든 경로를 포함하도록 부팅 구성을 업데이트합니다.
