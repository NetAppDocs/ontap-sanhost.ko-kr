...
Host Utilities 소프트웨어 패키지가 포함된 압축 파일을 에서 다운로드할 수 있습니다link:https://mysupport.netapp.com/site/products/all/details/hostutilities/downloads-tab/download/61343/6.1/downloads["NetApp Support 사이트"^]. 파일을 다운로드한 후 Host Utilities를 설치하는 데 필요한 두 가지 소프트웨어 패키지를 가져오기 위해 압축을 풀어야 합니다.
...
NetApp은 ONTAP LUN에 대해 다음과 같은 매개 변수 설정을 권장합니다. ONTAP 호스트 유틸리티 키트를 설치하면 LUN에 대한 중요 매개 변수가 자동으로 설정됩니다.
