...
summary: ONTAP와 함께 Oracle Linux 6.7을 사용하는 방법에 대해 설명합니다 
---
= ONTAP와 함께 Oracle Linux 6.7을 사용합니다
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
ONTAP SAN 호스트 구성 설정을 사용하여 ONTAP를 대상으로 Oracle Linux 6.7을 구성할 수 있습니다.
...
.필요한 것
SAN 부팅을 사용하려는 경우 구성에 의해 지원되어야 합니다. 를 사용할 수 있습니다 https://mysupport.netapp.com/matrix/imt.jsp?components=74908;74907;71545;71546;&solution=1&isHWU&src=IMT["NetApp 상호 운용성 매트릭스 툴"^] 사용 중인 OS, HBA, HBA 펌웨어 및 HBA 부팅 BIOS, ONTAP 버전이 지원되는지 확인합니다.
...
Oracle Linux 6.7의 경우 /etc/multipath.conf 파일이 있어야 하지만 파일을 구체적으로 변경할 필요는 없습니다. Oracle Linux 6.7은 ONTAP LUN을 인식하고 올바르게 관리하는 데 필요한 모든 설정으로 컴파일됩니다. ALUA 처리기를 활성화하려면 다음 단계를 수행하십시오.
...
Oracle Linux 6.7 OS는 ONTAP LUN을 인식하여 모든 구성 매개 변수를 올바르게 설정하도록 컴파일됩니다.
...
NOTE: Oracle Linux 6.7 RHCK(Red Hat Enterprise Kernel)을 구성하려면 link:hu_rhel_67.html#recommended-settings["권장 설정"]RHEL(Red Hat Enterprise Linux) 6.7용 을 사용하십시오.



== 알려진 문제

Oracle Linux 6.7 및 ONTAP 릴리즈에는 알려진 문제가 없습니다.


NOTE: Oracle Linux(Red Hat 호환 커널) 알려진 문제에 대해서는 을 참조하십시오 link:hu_rhel_67.html#known-problems-and-limitations["알려진 문제"] Red Hat Enterprise Linux(RHEL) 6.7의 경우
