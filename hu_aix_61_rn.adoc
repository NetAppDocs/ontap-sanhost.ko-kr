...
[role="lead"]
릴리스 노트 문서에는 알려진 문제, 제한 사항, ONTAP 스토리지 시스템 구성 및 관리에 대한 업데이트를 포함하여 AIX 호스트 유틸리티 6.1 릴리스에 대한 최신 정보가 포함되어 있습니다.

AIX Host Utilities 사용에 대한 새로운 정보가 공개되면 릴리즈 노트 문서가 업데이트됩니다.
