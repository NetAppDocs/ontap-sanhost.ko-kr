...
.RHEL 10.x 릴리스의 iopolicy 변경 사항
* RHEL 10.1부터 기본 iopolicy는 queue-depth입니다.
* RHEL 10.0에서는 라운드 로빈이 기본 iopolicy이지만 queue-depth는 구성 가능한 옵션으로 사용할 수 있습니다.


RHEL 10.0을 사용 중이고 iopolicy를 queue-depth로 변경하려면 udev 규칙 파일을 다음과 같이 수정하십시오.
...
.단계
. 호스트에서 각 ONTAP 네임스페이스에 대해 적절한 NVMe-oF 설정(예: 모델이 NetApp ONTAP 컨트롤러로 설정되고 로드 밸런싱 iopolicy가 queue-depth로 설정됨)이 올바르게 표시되는지 확인하십시오.
+
.. 하위 시스템을 표시합니다.
+
[source, cli]
