...
---
= NetApp ONTAP 기반의 Linux용 Veritas Infoscale 8 사용
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
이 문서에서는 FC, FCoE 및 iSCSI 프로토콜을 사용하는 Red Hat Enterprise Linux 및 Oracle Linux(RHCK 기반) 플랫폼용 VERITAS Infoscale 스토리지 기반 8 시리즈 릴리스에 대한 ONTAP SAN 호스트 구성 설정에 대한 지침을 제공합니다.
