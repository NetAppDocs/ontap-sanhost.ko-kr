...
summary: ONTAP 과 함께 Oracle Linux 8.x를 사용하는 방법을 설명합니다. 
---
= ONTAP 스토리지를 사용하여 FCP 및 iSCSI에 대한 Oracle Linux 8.x 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilities 소프트웨어는 ONTAP 스토리지에 연결된 Linux 호스트에 대한 관리 및 진단 도구를 제공합니다. Oracle Linux 8.x 호스트에 Linux 호스트 유틸리티를 설치하면 호스트 유틸리티를 사용하여 ONTAP LUN을 사용하여 FCP 및 iSCSI 프로토콜 작업을 관리할 수 있습니다.
...
Oracle Linux 8.x에서 멀티패스를 사용하여 ONTAP LUN을 관리할 수 있습니다.


NOTE: 당신은 사용할 수 있습니다 link:hu-rhel-8x.html#rhel-rhck["Red Hat Enterprise Linux(RHEL) 8.x에 권장되는 설정"] Oracle Linux 8.x에 대한 Red Hat 호환 커널을 구성합니다.
