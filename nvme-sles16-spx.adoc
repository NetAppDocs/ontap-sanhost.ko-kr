...
summary: ONTAP를 사용하여 SUSE Linux Enterprise Server 16에 대해 NVMe/FC를 구성하는 방법을 설명합니다. 
---
...
SUSE Linux Enterprise Server 16용 NVMe over Fabrics(NVMe-oF) 호스트 구성 방법을 알아보세요. 더 자세한 지원 및 기능 정보는 link:nvme-sles-supported-features.html["ONTAP 지원 및 기능"]을 참조하십시오.
...
.단계
. 서버에 SUSE Linux Enterprise Server 16을 설치하십시오. 설치가 완료되면 지정된 SUSE Linux Enterprise Server 16 커널이 실행 중인지 확인하십시오.
+
[source, cli]
----
uname -r
...
----
====
.. 커널 키링에서 TLS PSK를 내보냅니다. 이렇게 하면 호스트 재부팅 중에도 키가 유지됩니다.
+
[source, cli]
----
