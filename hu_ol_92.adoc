...
summary: ONTAP에서 Oracle Linux 9.2를 사용하는 방법에 대해 설명합니다 
---
= ONTAP와 함께 Oracle Linux 9.2를 사용하십시오
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
ONTAP SAN 호스트 구성 설정을 사용하여 ONTAP가 포함된 Oracle Linux 9.2를 대상으로 구성할 수 있습니다.
...
OL 9.2의 경우 `/etc/multipath.conf` 파일이 있어야 하지만 파일을 구체적으로 변경할 필요는 없습니다. ol 9.2는 ONTAP LUN을 인식하고 올바르게 관리하는 데 필요한 모든 설정으로 컴파일됩니다.
...
Oracle Linux 9.2 OS는 ONTAP LUN을 인식하여 ASA 및 비 ASA 구성 모두에 대해 모든 구성 매개 변수를 올바르게 자동 설정하도록 컴파일됩니다. 다음과 같은 권장 설정을 사용하여 호스트 구성의 성능을 더욱 최적화할 수 있습니다.
...
NOTE: Oracle Linux 9.2 RHCK(Red Hat Compatible Kernel)을 구성하려면 RHEL(Red Hat Enterprise Linux) 9.2용 을 link:hu_rhel_92.html#recommended-settings["권장 설정"]사용하십시오.
...
ONTAP 릴리즈가 포함된 Oracle Linux 9.2 에는 다음과 같은 알려진 문제가 있습니다.
...
SAN 호스트에서 Linux Host Utilities CLI 명령 "sanlun fcp show adapter -v"를 실행하면 명령이 실패하고 HBA(호스트 버스 어댑터) 검색에 필요한 라이브러리 종속성을 찾을 수 없다는 오류 메시지가 표시됩니다.
