...
summary: HPE VME 8.0.x 호스트에 Linux Host Utilities를 설치하여 ONTAP LUN을 사용한 FCP 및 iSCSI 프로토콜 작업을 관리하십시오. 
---
= ONTAP 스토리지를 사용하여 FCP 및 iSCSI를 위한 HPE VME 8.0.x 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilities 소프트웨어는 ONTAP 스토리지에 연결된 Linux 호스트를 위한 관리 및 진단 도구를 제공합니다. HPE VME 8.0.x 호스트에 Linux Host Utilities를 설치하면 Host Utilities를 사용하여 ONTAP LUN에서 FCP 및 iSCSI 프로토콜 작업을 관리할 수 있습니다.



[[step-1-install-the-linux-host-utilities]]
== 1단계: Linux Host Utilities 설치
...
HPE VME 8.0.x에서 다중 경로를 사용하여 ONTAP LUN을 관리할 수 있습니다.
...
* ONTAP LUN에 HPE VME 데이터 저장소를 구성한 다음 가상 머신을 구성하십시오. 자세한 내용은 HPE 공급업체 설명서를 참조하십시오.
