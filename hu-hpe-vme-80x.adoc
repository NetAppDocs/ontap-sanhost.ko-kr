...
호스트가 다른 벤더의 LUN에 연결되어 있고 다중 경로 파라미터 설정이 재정의된 경우, `multipath.conf` 파일에 ONTAP LUN에 특화된 스탠자를 추가하여 설정을 수정해야 합니다. 파일에서 아래로 스크롤하여 이러한 스탠자를 추가하십시오. 파라미터를 수정하지 않으면 ONTAP LUN이 예상대로 작동하지 않을 수 있습니다.
