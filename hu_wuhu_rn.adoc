...
| iSCSI 소프트웨어 부팅에서는 MCS(Multiple Connections per second)가 지원되지 않습니다 | iSCSI MCS를 사용하여 iSCSI 소프트웨어 부팅을 사용하는 경우 iSCSI 부트 선행 연결을 보호할 수 없습니다. 이로 인해 부팅 LUN 연결이 끊기고 호스트 시스템이 중단될 수 있습니다. | Microsoft는 iSCSI 소프트웨어 부팅 호스트에 대한 MCS를 지원하지 않습니다. MPIO를 고가용성 솔루션으로 사용합니다. MPIO와 MCS의 혼합은 지원되지 않습니다. 
|===
