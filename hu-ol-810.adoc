...
.시작하기 전에
를 사용하여 link:https://mysupport.netapp.com/matrix/imt.jsp?components=91863;&solution=1&isHWU&src=IMT["NetApp 상호 운용성 매트릭스 툴"^]OS, HBA, HBA 펌웨어, HBA 부팅 BIOS 및 ONTAP 버전이 지원되는지 확인합니다.
...
NOTE: Oracle Linux 8.10 RedHat Enterprise Kernel을 구성하려면 link:hu_rhel_810.html#recommended-settings["권장 설정"]RHEL(Red Hat Enterprise Linux) 8.10용 을 사용하십시오.



== KVM 설정

를 사용하여 KVM(커널 기반 가상 머신)을 구성할 수도 <<recommended_settings_ol_810,권장 설정>>있습니다. LUN이 하이퍼바이저에 매핑되므로 KVM을 구성하는 데 필요한 변경 사항은 없습니다.
