...
summary: ONTAP LUN을 사용하여 FCP 및 iSCSI 프로토콜 작업을 관리하기 위해 CentOS 8.1 호스트에 Linux 호스트 유틸리티를 설치합니다. 
---
= ONTAP 스토리지를 위한 FCP 및 iSCSI를 위한 CentOS 8.1 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux 호스트 유틸리티 소프트웨어는 ONTAP 스토리지에 연결된 Linux 호스트를 위한 관리 및 진단 툴을 제공합니다. CentOS 8.1 호스트에 Linux 호스트 유틸리티를 설치하면 호스트 유틸리티를 사용하여 ONTAP LUN을 사용하여 FCP 및 iSCSI 프로토콜 작업을 관리할 수 있습니다.
...
CentOS 8.1에서 멀티패스를 사용하여 ONTAP LUN을 관리할 수 있습니다.
...
ONTAP 스토리지를 사용하는 FCP 및 iSCSI의 경우 CentOS 8.1에 알려진 문제는 없습니다.

CentOS Red Hat 호환 커널의 알려진 문제는 다음을 참조하세요. link:hu_rhel_81.html#known-problems-and-limitations["알려진 문제"] Red Hat Enterprise Linux(RHEL) 8.1용입니다.
