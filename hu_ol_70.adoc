...
NOTE: Oracle Linux 7.0 RHCK(Red Hat Enterprise Kernel)를 구성하려면 RHEL(Red Hat Enterprise Linux) 7.0용 을 link:hu_rhel_70.html#recommended-settings["권장 설정"]사용하십시오.
...
| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=916501["916501"^] | IO를 통한 스토리지 페일오버 작업 중에 QLogic 10G FCoE(QLE8152) 호스트 커널 충돌이 관찰되었습니다 | 10G FCoE Qlogic(QLE8152) 호스트의 Qlogic 드라이버 모듈에서 커널 충돌이 관찰될 수 있습니다. 입출력의 스토리지 페일오버 작업 중에 충돌이 발생합니다. 이 충돌의 발생 가능성은 높아 호스트의 IO 중단 시간이 길어집니다. 
|===