...
---
= ONTAP 스토리지를 사용하여 FC, FCoE 및 iSCSI에 대해 Veritas Infoscale 6 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilities 소프트웨어는 ONTAP 스토리지에 연결된 Linux 호스트에 대한 관리 및 진단 도구를 제공합니다. ONTAP LUN을 사용하여 FC, FCoE 및 iSCSI 프로토콜 작업을 관리하는 기능을 지원하려면 Veritas Infoscale 6 for Oracle Linux(Red Hat 호환 커널 기반), Red Hat Enterprise Linux(RHEL) 및 SUSE Linux Enterprise Server 호스트와 함께 Linux 호스트 유틸리티를 사용하세요.
...
Veritas Infoscale 6과 함께 Veritas Dynamic Multipathing(VxDMP)을 사용하여 ONTAP LUN을 관리합니다.
