...
[role="lead"]
FC, FCoE 및 iSCSI 프로토콜을 사용하는 Red Hat Enterprise Linux 및 Oracle Linux(Red Hat 호환 커널 기반) 플랫폼용 Veritas Storage Foundation 6 시리즈 릴리스에 ONTAP SAN 호스트 구성 설정을 사용할 수 있습니다.
...
구성이 시스템 요구 사항을 충족하는지 확인해야 합니다. 자세한 내용은 https://imt.netapp.com/matrix/#welcome["상호 운용성 매트릭스 툴"^] 및 Veritas HCL 매트릭스를 참조하십시오.
