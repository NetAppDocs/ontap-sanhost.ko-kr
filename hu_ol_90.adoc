...
이 툴킷은 NetApp Host Utilities 패키지를 설치할 때 자동으로 설치됩니다. 이 키트는 LUN 및 HBA를 관리하는 데 도움이 되는 sandlun 유틸리티를 제공합니다. sandlun 명령은 호스트에 매핑된 LUN, 다중 경로 및 이니시에이터 그룹을 생성하는 데 필요한 정보에 대한 정보를 반환합니다.

다음 예제에서 sanlun lun show 명령은 LUN 정보를 반환합니다.
...
Oracle Linux 9.0에는 알려진 문제가 없습니다.
