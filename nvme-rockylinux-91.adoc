...
summary: ONTAP을 사용하여 Rocky Linux 9.1에 NVMe-oF 호스트를 구성하는 방법 
---
= ONTAP 스토리지를 사용하여 NVMe-oF에 Rocky Linux 9.1 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Rocky Linux 9.1에 대해 NVMe over Fabrics(NVMe-oF) 호스트를 구성하는 방법을 알아보세요. 추가 지원 및 기능 정보는 다음을 참조하세요. link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP 지원 및 기능"].

*Rocky Linux 9.1을 사용하는 NVMe-oF에는 다음과 같은 알려진 제한 사항이 있습니다.*
...
.단계
. 서버에 Rocky Linux 9.1을 설치합니다.  설치가 완료되면 필요한 Rocky Linux 9.1 커널을 실행하고 있는지 확인하세요.
+
[source, cli]
----
uname -r
...
| 1503468 | `nvme list-subsys` 명령은 주어진 하위 시스템에 대해 나열된 반복되는 NVMe 컨트롤러를 반환합니다. | 그만큼  `nvme list-subsys` 명령은 주어진 하위 시스템과 연결된 NVMe 컨트롤러의 고유한 목록을 반환해야 합니다. Rocky Linux 9.1에서는  `nvme list-subsys` 이 명령은 지정된 서브시스템에 속하는 모든 네임스페이스에 대해 해당 ANA 상태와 함께 NVMe 컨트롤러를 반환합니다. 그러나 ANA 상태는 네임스페이스별 속성이므로, 지정된 네임스페이스에 대한 서브시스템 명령 구문을 나열하는 경우 경로 상태를 포함하는 고유한 NVMe 컨트롤러 항목이 표시되어야 합니다. 
|===