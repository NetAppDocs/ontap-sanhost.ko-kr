...
summary: ONTAP와 함께 Red Hat Enterprise Linux 8.7을 사용하는 방법을 설명합니다 
---
= NetApp ONTAP와 함께 Red Hat Enterprise Linux 8.7 사용
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
RHEL(Red Hat Enterprise Linux) 8.7의 경우 `/etc/multipath.conf` 파일이 있어야 하지만 파일을 구체적으로 변경할 필요는 없습니다. RHEL 8.7은 ONTAP LUN을 인식하고 올바르게 관리하는 데 필요한 모든 설정으로 컴파일됩니다.
...
RHEL 8.7 OS는 ONTAP LUN을 인식하여 ASA 및 비 ASA 구성 모두에 대해 모든 구성 매개 변수를 올바르게 자동 설정하도록 컴파일됩니다.
...
| 1501048 | FC 전송 계층 시간 초과 매개 변수입니다 `dev_loss_tmo` 원격 SCSI 타겟 포트의 경우 음수 값(-1)을 보고합니다. |  `dev_loss_tmo` timeout 매개 변수는 FC 전송이 원격 포트의 손실을 방지하기 위해 절연되어야 하는 기본 시간(초)을 나타냅니다. 이 값을 초과하면 SCSI 타겟이 호스트에서 제거됩니다. 이 매개 변수 값은 양의 정수 값이어야 합니다. 하지만 RHEL 8.7에서는 `sysfs` 파일 시스템은 에 대해 음수 값(-1)을 반환합니다 `dev_loss_tmo` SCSI 디바이스에 대한 시간 초과 매개 변수입니다. 다음은 를 확인하는 명령줄 예입니다 `dev_loss_tmo` 작업 호스트의 매개 변수:
`# cat /sys/devices/pci*/*/*/host*/rport*/fc_remote_ports/rport*/dev_loss_tmo
4294967295`. RHEL 8.7에서는 이 문제에 대한 해결 방법이 없습니다. | link:https://bugzilla.redhat.com/show_bug.cgi?id=2124297["2124297"^] 
|===
