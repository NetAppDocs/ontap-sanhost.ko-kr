...
* ONTAP 9.16.1 버전부터 새로 생성되는 모든 NVMe 네임스페이스에 대해 공간 할당이 기본적으로 활성화됩니다.
* ONTAP 9.9.1 P3 버전부터 ESXi 8에서 NVMe/FC 프로토콜이 지원됩니다.
* ONTAP 9.10.1부터 NVMe/TCP 프로토콜이 ONTAP에 대해 지원됩니다.
...
| link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1420654["1420654)를 참조하십시오"^] | ONTAP 버전 9.9.1에서 NVMe/FC 프로토콜을 사용할 때 ONTAP 노드가 작동하지 않습니다 | ONTAP 9.9.1에서는 NVMe "abort" 명령에 대한 지원이 도입되었습니다. ONTAP가 파트너 명령을 기다리는 NVMe 융합 명령을 중단하기 위한 "abort" 명령을 수신하면 ONTAP 노드 중단이 발생합니다. 이 문제는 NVMe 융합 명령(예: ESX)과 파이버 채널(FC) 전송을 사용하는 호스트에서만 발견됩니다. 


| link:https://jira.ngage.netapp.com/browse/CONTAPEXT-1126["CONTAPEXT-1126"^] | vNVMe 어댑터를 사용하는 Linux VM에서 긴 모든 경로 중단(APD) 창이 발생할 때 I/O 오류가 발생합니다  a| 
Linux VM이 vSphere 8.x 이상을 실행하고 가상 NVMe(vNVMe) 어댑터를 사용하는 경우 vNVMe 재시도 작업이 기본적으로 비활성화되어 있어 I/O 오류가 발생합니다. 모든 경로 다운(APD) 또는 과도한 I/O 부하 시 이전 커널을 실행하는 Linux VM에서 중단을 방지하기 위해 VMware는 vNVMe 재시도 작업을 비활성화하는 튜닝 가능한 "VSCSIDisableNvmeRetry"를 도입했습니다.
