...
.단계
. 서버에 RHEL 8.6를 설치합니다. 설치가 완료되면 지정된 RHEL 8.6 커널을 실행하고 있는지 확인합니다. 지원되는 버전의 최신 목록은 를 link:https://mysupport.netapp.com/matrix/["상호 운용성 매트릭스 툴"^]참조하십시오.
. 설치가 완료되면 지정된 RHEL 8.6 커널을 실행하고 있는지 확인합니다. 지원되는 버전의 최신 목록은 를 link:https://mysupport.netapp.com/matrix/["상호 운용성 매트릭스 툴"^]참조하십시오.
+
예:
...
--
RHEL 8.6 커널에 포함된 기본 받은 편지함 `qla2xxx` 드라이버에는 최신 업스트림 수정 사항이 있습니다. 이러한 수정 사항은 ONTAP 지원에 필수적입니다.
