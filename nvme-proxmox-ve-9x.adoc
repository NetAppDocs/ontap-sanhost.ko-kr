...
summary: ONTAP 스토리지를 사용하는 Proxmox VE 9.x용 NVMe-oF 호스트 구성 방법 
---
...
Proxmox VE 9.x용 NVMe over Fabrics(NVMe-oF) 호스트를 구성하는 방법을 알아보세요. 추가 지원 및 기능 정보는 다음을 참조하세요. link:nvme-proxmox-supported-features.html["ONTAP 지원 및 기능"^].
...
.단계
. 서버에 Proxmox VE 9.x를 설치하세요.  설치가 완료되면 필요한 Proxmox VE 9.x 커널이 실행 중인지 확인하십시오.
+
[source, cli]
----
uname -r
...
Proxmox VE 9.0부터 NVMe-oF의 기본 iopolicy는 `queue-depth`로 설정되어 있습니다. iopolicy를 `round-robin`로 변경하려면(이 또한 구성 가능한 옵션으로 제공됨) udev 규칙 파일을 다음과 같이 수정하십시오:
...
----
. 다음 예시와 같이 규칙을 수정하여 `queue-depth`를 `round-robin`로 변경하십시오:
+
[listing]
----
...
----
. 서브시스템의 현재 iopolicy를 확인하십시오. <subsystem>를 서브시스템 이름으로 바꾸십시오(예: nvme-subsys0).
+
[source, cli]
----
