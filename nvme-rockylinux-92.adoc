...
summary: ONTAP을 사용하여 Rocky Linux 9.2에 NVMe-oF 호스트를 구성하는 방법 
---
= ONTAP 스토리지를 사용하여 NVMe-oF에 Rocky Linux 9.2 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Rocky Linux 9.2에 대해 NVMe over Fabrics(NVMe-oF) 호스트를 구성하는 방법을 알아보세요. 추가 지원 및 기능 정보는 다음을 참조하세요. link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP 지원 및 기능"].

*Rocky Linux 9.2를 사용하는 NVMe-oF에는 다음과 같은 알려진 제한 사항이 있습니다.*
...
.단계
. 서버에 Rocky Linux 9.2를 설치합니다.  설치가 완료되면 필요한 Rocky Linux 9.2 커널을 실행하고 있는지 확인하세요.
+
[source, cli]
----
uname -r
