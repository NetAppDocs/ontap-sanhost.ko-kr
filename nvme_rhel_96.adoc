...
summary: ONTAP가 포함된 RHEL 9.6용 NVMe-oF 호스트를 구성하는 방법 
---
= ONTAP 탑재 RHEL 9.6에 대한 NVMe-oF 호스트 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
.이 작업에 대해
RHEL(Red Hat Enterprise Linux) 9.6용 NVMe-oF 호스트 구성에서 다음과 같은 지원 및 기능을 사용할 수 있습니다. 또한 구성 프로세스를 시작하기 전에 알려진 제한 사항을 검토해야 합니다.
...
** RHEL 9.6는 기본적으로 NVMe 네임스페이스를 위한 커널 내 NVMe 다중 경로를 지원하므로 명시적인 설정이 필요하지 않습니다.
** ONTAP 9.12.1부터 NVMe/TCP에 대한 보안 인밴드 인증이 지원됩니다. RHEL 9.6에서는 NVMe/TCP에 대한 보안 인밴드 인증을 사용할 수 있습니다.
+
...
다음 절차에 따라 지원되는 최소 RHEL 9.6 소프트웨어 버전을 확인하세요.

.단계
. 서버에 RHEL 9.6를 설치합니다. 설치가 완료되면 지정된 RHEL 9.6 커널을 실행하고 있는지 확인합니다.
+
[source, cli]
----
uname -r
...
----
. RHEL 9.6 호스트에서 다음 위치에서 hostnqn 문자열을 `/etc/nvme/hostnqn` 확인합니다.
+
[source, cli]
----
...
RHEL 9.6에서는  `nvmefc-boot-connections.service` 그리고  `nvmf-autoconnect.service` NVMe/FC에 포함된 부팅 서비스  `nvme-cli` 패키지는 시스템 부팅 시 자동으로 시작되도록 설정됩니다. 시스템 부팅이 완료되면 부팅 서비스가 활성화되었는지 확인하세요.
...
NOTE: RHEL 9.5부터는 NVMe/TCP 시간 제한에 대한 기본 설정이 `ctrl_loss_tmo` 꺼집니다. 다시 시도 횟수에 제한이 없음을 의미합니다(무제한 재시도). 따라서 또는 `nvme connect-all` 명령(option-l)을 사용할 때 특정 시간 초과 기간을 `nvme connect` 수동으로 구성할 필요가 `ctrl_loss_tmo` 없습니다. 이 기본 동작에서는 경로 장애 발생 시 NVMe/TCP 컨트롤러가 시간 초과를 경험하지 않고 무기한 연결된 상태를 유지합니다.
...
ONTAP 9.12.1부터 RHEL 9.6 호스트와 ONTAP 컨트롤러 간의 NVMe/TCP를 통한 안전한 인밴드 인증이 지원됩니다.
...
cat /etc/nvme/hostnqn
----
. RHEL 9.6 호스트에 대한 dhchap 키를 생성합니다.
+
