...
RHEL 9.6을 사용하면  `nvmefc-boot-connections.service` 그리고  `nvmf-autoconnect.service` NVMe/FC에 포함된 부팅 서비스  `nvme-cli` 패키지는 시스템 부팅 시 자동으로 활성화됩니다. 부팅이 완료되면 서비스가 활성화되었는지 확인하세요.
