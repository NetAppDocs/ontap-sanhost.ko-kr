...
summary: ONTAP 사용하여 RHEL 9.x에 NVMe-oF 호스트를 구성하는 방법 
---
= ONTAP 스토리지를 사용하여 NVMe-oF에 대한 RHEL 9.x 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
RHEL 9.x에 대해 NVMe over Fabrics(NVMe-oF) 호스트를 구성하는 방법을 알아보세요. 추가 지원 및 기능 정보는 다음을 참조하세요. link:nvme-rhel-supported-features.html["RHEL ONTAP 지원 및 기능"].

RHEL 9.x를 사용하는 NVMe-oF에는 다음과 같은 알려진 제한 사항이 있습니다.
...
.단계
. 서버에 RHEL 9.x를 설치합니다. 설치가 완료되면 필요한 RHEL 9.x 커널을 실행하고 있는지 확인하세요.
+
[source, cli]
----
uname -r
...
.RHEL 9.x 릴리스의 iopolicy 변경 사항
* RHEL 9.7부터 기본 iopolicy는 queue-depth입니다.
* RHEL 9.6에서는 라운드 로빈이 기본 iopolicy이지만 queue-depth는 구성 가능한 옵션으로 사용할 수 있습니다.
* RHEL 9.6 이전에는 round-robin이 유일하게 지원되는 NVMe-oF iopolicy입니다.


RHEL 9.6을 사용 중이고 iopolicy를 `queue-depth`로 변경하려면 udev 규칙 파일을 다음과 같이 수정하십시오.
...
RHEL 9.x 호스트와 ONTAP 컨트롤러 간의 NVMe/TCP를 통해 안전한 인밴드 인증이 지원됩니다.
...
cat /etc/nvme/hostnqn
----
. RHEL 9.x 호스트에 대한 dhchap 키를 생성합니다.
+
...
| 1503468 | RHEL 9.1에서는 `nvme list-subsys` 명령은 주어진 하위 시스템에 대한 반복된 NVME 컨트롤러 목록을 반환합니다. | 그만큼 `nvme list-subsys` 명령은 주어진 하위 시스템에 대한 NVMe 컨트롤러 목록을 반환합니다.  RHEL 9.1에서 이 명령은 하위 시스템의 모든 네임스페이스에 대한 ANA 상태와 함께 컨트롤러를 표시합니다.  ANA 상태는 네임스페이스별 속성이므로 명령은 각 네임스페이스의 경로 상태를 포함하는 고유한 컨트롤러 항목을 표시해야 합니다. 


| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | RHEL 9.0 NVMe-oF 호스트는 중복된 PDC(Persistent Discovery Controller)를 생성합니다. | NVMe-oF 호스트에서   `nvme discover -p` 명령을 사용하여 PDC를 생성할 수 있습니다. 이 명령을 사용하면 개시자-대상 조합당 하나의 PDC만 생성되어야 합니다. 그러나 NVMe-oF 호스트를 사용하여 ONTAP 9.10.1 및 RHEL 9.0을 실행하는 경우 `nvme discover -p` 가 실행될 때마다 중복 PDC가 생성됩니다. 이로 인해 호스트와 대상 모두에서 불필요한 리소스 사용이 발생합니다. 
|===