...
Red Hat Enterprise Linux(RHEL) 9.5에 대해 NVMe over Fabrics(NVMe-oF) 호스트를 구성하는 방법을 알아보세요.  추가 지원 및 기능 정보는 다음을 참조하세요.link:hu-nvme-index.html["NVME-oF 개요"^] .

RHEL 9.5의 NVMe-oF에는 알려진 제한 사항이 없습니다.
...
.단계
. 서버에 RHEL 9.5를 설치합니다.  설치가 완료되면 필요한 RHEL 9.5 커널을 실행하고 있는지 확인하세요.
+
[source, cli]
----
uname -r
