...
summary: ONTAP와 함께 Red Hat Enterprise Linux 7.7을 사용하는 방법을 설명합니다 
---
= NetApp ONTAP와 함께 Red Hat Enterprise Linux 7.7 사용
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
.필요한 것
SAN 부팅을 사용하려는 경우 구성에 의해 지원되어야 합니다. 를 사용할 수 있습니다 link:https://mysupport.netapp.com/matrix/imt.jsp?components=89085;&solution=1&isHWU&src=IMT["NetApp 상호 운용성 매트릭스 툴"^] 사용 중인 OS, HBA, HBA 펌웨어 및 HBA 부팅 BIOS, ONTAP 버전이 지원되는지 확인합니다.
...
RHEL(Red Hat Enterprise Linux) 7.7의 경우 /etc/multipath.conf 파일이 있어야 하지만 파일을 구체적으로 변경할 필요는 없습니다. RHEL 7.7은 ONTAP LUN을 인식하고 올바르게 관리하는 데 필요한 모든 설정으로 컴파일됩니다.
...
RHEL 7.7 OS는 ONTAP LUN을 인식하여 ASA 및 비 ASA 구성 모두에 대해 모든 구성 매개 변수를 올바르게 자동 설정하도록 컴파일됩니다.
...
| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=1258856["1258856을 참조하십시오"^] | 스토리지 페일오버 작업 중 Emulex LPe16002 16GB FC를 사용하는 RHEL7U7에서 원격 포트가 차단됨 상태로 전환되었습니다 | 스토리지 페일오버 작업 중에 원격 포트가 LPe16002 16GB FC 어댑터가 있는 RHEL 7.7 호스트에서 차단됨 상태로 전환될 수 있습니다. 스토리지 노드가 최적 상태로 돌아가면 LIF도 함께 올라가고 원격 포트 상태가 "온라인"으로 표시되어야 합니다. 원격 포트 상태가 계속 "차단됨" 또는 "없음"으로 표시될 수 있습니다. 이 상태에서는 다중 경로 계층의 LUN에 대한 "장애 발생" 경로가 발생할 수 있습니다. | link:https://bugzilla.redhat.com/show_bug.cgi?id=1743667["1743667"^] 


| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=1261474["1261474"^] | Emulex LPe32002 32Gb FC를 사용하는 RHEL7U7에서 원격 포트를 차단 상태로 전환합니다 | 스토리지 페일오버 작업 중에 원격 포트가 LPe32002 32Gb FC 어댑터가 있는 RHEL 7.7 호스트에서 차단됨 상태로 전환될 수 있습니다. 스토리지 노드가 최적 상태로 돌아가면 LIF도 함께 올라가고 원격 포트 상태가 "온라인"으로 표시되어야 합니다. 원격 포트 상태가 계속 "차단됨" 또는 "없음"으로 표시될 수 있습니다. 이 상태에서는 다중 경로 계층의 LUN에 대한 "장애 발생" 경로가 발생할 수 있습니다. | link:https://bugzilla.redhat.com/show_bug.cgi?id=1745995["1745995"^] 
|===
