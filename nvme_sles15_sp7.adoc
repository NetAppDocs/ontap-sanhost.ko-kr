...
[role="lead"]
SUSE Linux Enterprise Server 호스트는 비대칭 네임스페이스 액세스(ANA)를 통해 NVMe over Fibre Channel(NVMe/FC) 및 NVMe over TCP(NVMe/TCP) 프로토콜을 지원합니다.  ANA는 iSCSI 및 FCP 환경에서 ALUA(비대칭 논리 장치 액세스)와 동일한 다중 경로 기능을 제공합니다.

.이 작업에 대해
SUSE Linux Enterprise Server 15 SP7의 NVMe-oF 호스트 구성에서는 다음 지원 및 기능을 사용할 수 있습니다. 구성 과정을 시작하기 전에 알려진 제한 사항도 검토해야 합니다.
...
** ONTAP 9.12.1부터 NVMe/TCP 및 NVMe/FC에 대한 안전한 인밴드 인증 지원이 도입되었습니다.  SUSE Linux Enterprise Server 15 SP7에서는 NVMe/TCP 및 NVMe/FC에 대한 안전한 인밴드 인증을 사용할 수 있습니다.
** 고유한 검색 NQN을 사용하여 지속적 검색 컨트롤러(PDC)를 지원합니다.
** NVMe/TCP에 대한 TLS 1.3 암호화 지원.
** NetApp `sanlun` SUSE Linux Enterprise Server15 SP7 호스트에서는 NVMe-oF에 대한 호스트 유틸리티 지원을 사용할 수 없습니다.  대신, 기본으로 포함된 NetApp 플러그인을 사용할 수 있습니다. `nvme-cli` 모든 NVMe-oF 전송을 위한 패키지입니다.
+
...
다음 절차에 따라 지원되는 최소 SUSE Linux Enterprise Server 15 SP7 소프트웨어 버전을 확인하세요.

.단계
. 서버에 SUSE Linux Enterprise Server15 SP7을 설치합니다.  설치가 완료되면 지정된 SUSE Linux Enterprise Server 15 SP7 커널을 실행하고 있는지 확인하세요.
+
[source, cli]
----
uname -r
----
+
다음 예는 SUSE Linux Enterprise Server 커널 버전을 보여줍니다.
...
SUSE Linux Enterprise Server 15 SP7을 사용하면 `nvmefc-boot-connections.service` 그리고 `nvmf-autoconnect.service` NVMe/FC에 포함된 부팅 서비스 `nvme-cli` 패키지는 시스템 부팅 중에 자동으로 시작되도록 설정됩니다.  시스템 부팅이 완료되면 부팅 서비스가 활성화되었는지 확인하세요.
...
NOTE: SUSE Linux Enterprise Server 15 SP6부터 NVMe/TCP의 기본 설정은 다음과 같습니다. `ctrl-loss-tmo` 시간 초과가 해제되었습니다.  이는 재시도 횟수에 제한이 없고(무기한 재시도) 특정 재시도를 수동으로 구성할 필요가 없음을 의미합니다. `ctrl-loss-tmo` 사용 시 시간 초과 기간 `nvme connect` 또는 `nvme connect-all` 명령(옵션 `-l` ).  또한, NVMe/TCP 컨트롤러는 경로 장애가 발생해도 시간 초과가 발생하지 않으며 무기한 연결 상태를 유지합니다.
...
ONTAP 9.11.1부터 SUSE Linux Enterprise Server 15 SP7 호스트에 대한 영구 검색 컨트롤러(PDC)를 생성할 수 있습니다.  PDC는 NVMe 하위 시스템 추가 또는 제거 작업과 검색 로그 페이지 데이터의 변경 사항을 자동으로 감지하는 데 필요합니다.
