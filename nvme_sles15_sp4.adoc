...
---
= ONTAP가 포함된 SUSE Linux Enterprise Server 15 SP4용 NVMe-of 호스트 구성
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
NVMe over Fabrics 또는 NVMe-oF(NVMe/FC 및 기타 전송 포함)는 ANA(비대칭 네임스페이스 액세스)가 포함된 SUSE Linux Enterprise Server 15 SP4(SLES15 SP4)에서 지원됩니다. ANA는 NVMe-oF 환경에서 ALUA와 동등한 제품이며, 현재 in-kernel NVMe Multipath로 구현되고 있습니다. SLES15 SP4 및 ONTAP에서 표적으로 ANA를 사용하여 커널 내 NVMe 다중 경로를 사용하여 NVMe-oF를 활성화하는 방법에 대한 자세한 내용은 여기 에 설명되어 있습니다.
...
* SLES15 SP4는 NVMe/FC 및 기타 전송을 지원합니다.
* NVMe-oF에 대한 선언은 지원되지 않습니다. 따라서 SLES15 SP4의 NVMe-of에 대한 LUHU는 지원되지 않습니다. 대신 기본 'NVMe-CLI'에 포함된 NetApp 플러그인을 사용할 수 있습니다. 이 작업은 모든 NVMe-oF 전송에서 작동합니다.
* NVMe와 SCSI 트래픽을 모두 동일한 동일한 호스트에서 실행할 수 있습니다. 실제로 이는 일반적으로 배포되는 호스트 구성일 것으로 예상됩니다. 따라서 SCSI의 경우 SCSI LUN에 대해 평소처럼 dm-multipath를 구성하여 mpath 디바이스를 생성할 수 있지만, NVMe multipath를 사용하여 호스트에 NVMe-of multipath 장치를 구성할 수 있습니다.
...
SLES15 SP4와 같은 SLES 호스트에서는 커널 내 NVMe 다중 경로가 기본적으로 활성화되어 있습니다. 따라서 여기서는 추가 설정이 필요하지 않습니다. 을 참조하십시오 link:https://mysupport.netapp.com/matrix/["NetApp 상호 운용성 매트릭스"^] 지원되는 구성에 대한 정확한 세부 정보
...
* SLES15 SP4 MU 호스트에 필수 커널과 'NVMe-CLI' 유지 관리 업데이트(MU) 패키지가 설치되어 있는지 확인합니다.
+
예:
...
호스트에서 사용 중인 FC 어댑터에 따라 다음 단계를 수행합니다.
...
----
+
최신 lpfc 드라이버(수신함 및 아웃박스 모두)에는 이미 'lpfc_enable_fc4_type' 기본값이 3으로 설정되어 있으므로 더 이상 '/etc/modprobe.d/lpfc.conf'에서 명시적으로 설정하고 'initrd'를 다시 생성할 필요가 없습니다. lpfc NVMe 지원은 기본적으로 이미 활성화되어 있습니다.
...
----
+
기존의 기본 받은 편지함 lpfc 드라이버는 이미 최신 버전이며 NVMe/FC와 호환됩니다. 따라서 lpfc Out of Box(OOB) 드라이버를 설치할 필요가 없습니다.
...
Online
----
. NVMe/FC 이니시에이터 포트가 활성화되어 있고 타겟 포트를 볼 수 있으며 모두 작동 및 실행 중인지 확인합니다. 이 예에서는 출력에 표시된 대로 이니시에이터 포트 1개만 사용하도록 설정되고 2개의 타겟 LIF에 연결됩니다.
+
[listing]
----
...
최신 SLES15 SP4 MU 커널에 포함된 기본 받은 편지함 qla2xxx 드라이버에는 ONTAP 지원에 필수적인 최신 업스트림 픽스가 포함되어 있습니다.
...
* 경로 복구 후 자동 재연결 안 함 NVMe/TCP는 경로 다운 후 10분 동안 기본 'Ctrl-Loss-TMO' 타이머 이후에 복구된 경로에 자동으로 다시 연결할 수 없습니다.
* 호스트 부팅 중 자동 연결 없음 NVMe/TCP는 호스트 부팅 중에도 자동으로 연결할 수 없습니다.
...
----
. 실행 `nvme connect-all` 노드를 통해 지원되는 모든 NVMe/TCP 이니시에이터-타겟 LIF에 대해 명령을 실행합니다. 를 더 길게 설정하십시오 `ctrl_loss_tmo` 에서 타이머 재시도 기간(예: 30분, -l 1800까지 설정 가능)입니다 `connect-all` 따라서 경로 손실이 발생할 경우 더 오랜 시간 동안 재시도합니다. 예를 들면, 다음과 같습니다.
+
[listing]
----
...
Y
----
. 각 ONTAP 네임스페이스에 대한 적절한 NVMe-of 설정(예: 모델을 NetApp ONTAP 컨트롤러로 설정 및 로드 밸런싱 IPolicy를 라운드 로빈으로 설정)이 호스트에 올바로 반영되는지 확인합니다.
+
[listing]
----
...
----
* 타겟 LIF가 "NVMe_info" 출력에서 위와 같이 표시되지 않으면 의심되는 NVMe/FC 오류에 대한 '/var/log/messages' 및 'mesg' 출력을 확인하여 그에 따라 보고 또는 수정하십시오.
