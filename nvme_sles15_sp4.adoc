...
저장소 장애 조치(SFO) 같은 ONTAP 장애 조치 이벤트를 편안하게 넘기 위해서는 Ctrl_Loss_TMO 타이머를 조정하여 30분 등의 더 긴 재시도 기간을 설정하는 것이 좋습니다. 다음은 세부 정보입니다.
...
----
. 노드를 통해 지원되는 모든 NVMe/TCP 이니시에이터-타겟 LIF에서 'NVMe connect-all' 명령을 실행합니다. 'CONNECT-ALL'이 진행되는 동안 'Ctrl_Loss_TMO' 기간(예: 30분 ~ -l 1800)을 더 길게 전달하여 경로 손실이 발생할 경우 더 오랜 기간 동안 재시도하도록 해야 합니다. 예를 들면, 다음과 같습니다.
+
[listing]
----
