...
NVMe over Fabrics 또는 NVMe-oF(NVMe/FC 및 기타 전송 포함)는 ANA(Asymmetric Namespace Access)가 있는 RHEL 8.7에서 지원됩니다. ANA는 NVMe-oF 환경에 해당하는 ALUA로, 현재 in-kernel NVMe Multipath로 구현되고 있습니다. RHEL 8.7에서 ANA를 사용하고 ONTAP를 대상으로 커널 내 NVMe 다중 경로를 사용하여 NVMe-oF를 활성화하는 방법에 대한 자세한 내용은 여기 에 설명되어 있습니다.
...
.단계
. RHEL 8.7을 서버에 설치합니다. 설치가 완료되면 지정된 RHEL 8.7 커널을 실행 중인지 확인합니다. 를 참조하십시오 link:https://mysupport.netapp.com/matrix/["NetApp 상호 운용성 매트릭스"^] 를 참조하십시오.
...
| 1479047 | RHEL 8.x NVMe-of 호스트는 중복된 영구 검색 컨트롤러를 만듭니다 | NVMe-oF(NVMe over Fabrics) 호스트에서는 를 제공하여 PDC(영구적 검색 컨트롤러)를 생성할 수 있습니다 `-p` 에 대한 옵션 `nvme discover` 명령. 지정된 초기자-대상 조합의 경우 각 호출을 통해 PDC 하나를 만들어야 합니다 `nvme discover` 명령. 그러나 ONTAP 9.10.1.1부터 RHEL(Red Hat Enterprise Linux) 8.x NVMe-of 호스트의 경우 각 호출을 통해 중복 PDC가 생성됩니다 `nvme discover` 명령과 함께 `-p` 옵션을 선택합니다. 이로 인해 호스트와 타겟 모두에서 리소스가 낭비됩니다. | 2087000 
|===
