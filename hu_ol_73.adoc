...
Oracle Linux 7.3 OS는 ONTAP LUN을 인식하여 모든 구성 매개 변수를 올바르게 설정합니다.  `multipath.conf`다중 경로 데몬을 시작하려면 파일이 있어야 합니다. 이 파일이 없는 경우 명령을 사용하여 0바이트 빈 파일을 만들 수 `touch /etc/multipath.conf` 있습니다.
...
NOTE: Oracle Linux 7.3 RHCK(Red Hat Compatible Kernel)을 구성하려면 RHEL(Red Hat Enterprise Linux) 7.3용 을 link:hu_rhel_73.html#recommended-settings["권장 설정"]사용하십시오.
...
Oracle Linux 7.3과 ONTAP 릴리스의 경우 알려진 문제가 없습니다.
