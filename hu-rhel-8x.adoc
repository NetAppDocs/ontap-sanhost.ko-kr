...
summary: ONTAP LUN을 사용하여 FCP 및 iSCSI 프로토콜 작업을 관리하려면 RHEL 8.x 호스트에 Linux 호스트 유틸리티를 설치합니다. 
---
= ONTAP 스토리지를 사용하여 FCP 및 iSCSI에 대한 RHEL 8.x 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilities 소프트웨어는 ONTAP 스토리지에 연결된 Linux 호스트에 대한 관리 및 진단 도구를 제공합니다. Red Hat Enterprise Linux(RHEL) 8.x 호스트에 Linux 호스트 유틸리티를 설치하면 호스트 유틸리티를 사용하여 ONTAP LUN을 사용하여 FCP 및 iSCSI 프로토콜 작업을 관리할 수 있습니다.
...
RHEL 8.x에서는 멀티패스를 사용하여 ONTAP LUN을 관리할 수 있습니다.
...
== 5단계: 선택적으로 다중 경로에서 장치를 제외할 수 있습니다.
...
== 6단계: ONTAP LUN에 대한 다중 경로 매개 변수 사용자 지정

호스트가 다른 벤더의 LUN에 연결되어 있고 다중 경로 파라미터 설정이 재정의된 경우, 기존 스탠자를 수정하거나 ONTAP LUN에 특화된 새 스탠자를 `multipath.conf` 파일에 추가하여 설정을 수정해야 합니다. 파일에서 아래로 스크롤하여 호스트에 적용되는 변경 사항을 확인하십시오. 파라미터를 수정하지 않으면 ONTAP LUN이 예상대로 작동하지 않을 수 있습니다.
...
ONTAP 스토리지를 탑재한 RHEL 8.x에는 다음과 같은 알려진 문제가 있습니다.
...
| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=1275843["1275843)을 참조하십시오"^] | 스토리지 페일오버 작업 중 QLogic QLE2672 16GB FC HBA를 사용하는 Red Hat Enterprise Linux 8.1에서 커널 중단이 발생할 수 있습니다 | QLogic QLE2672 FC(Fibre Channel) HBA(호스트 버스 어댑터)가 있는 Red Hat Enterprise Linux 8.1 커널에서 스토리지 페일오버 작업 중에 커널 중단이 발생할 수 있습니다. 커널이 중단되면 Red Hat Enterprise Linux 8.1이 재부팅되어 애플리케이션이 중단됩니다. kdump 메커니즘이 활성화된 경우 커널 중단은 /var/crash/ 디렉토리에 있는 vmcore 파일을 생성합니다. vmcore 파일을 검사하여 중단 원인을 확인할 수 있습니다. QLogic QLE2672 HBA 이벤트를 사용한 스토리지 페일오버는 "kmem_cache_alloc+131" 모듈에 영향을 줍니다. 커널 중단 후 "[exception RIP:kmem_cache_alloc+131]" 문자열을 찾아 vmcore 파일에서 이벤트를 찾을 수 있습니다. 호스트 OS를 재부팅하고 운영 체제를 복구합니다. 그런 다음 응용 프로그램을 다시 시작합니다 


| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=1275838["1275838)을 참조하십시오"^] | 스토리지 페일오버 작업 중 QLogic QLE2742 32Gb FC HBA가 장착된 Red Hat Enterprise Linux 8.1에서 커널 중단이 발생합니다 | 커널 중단은 QLogic QLE2742 FC(Fibre Channel) HBA(호스트 버스 어댑터)가 있는 Red Hat Enterprise Linux 8.1 커널에서 스토리지 페일오버 작업 중에 발생합니다. 커널이 중단되면 Red Hat Enterprise Linux 8.1이 재부팅되어 애플리케이션이 중단됩니다. kdump 메커니즘이 활성화된 경우 커널 중단은 /var/crash/ 디렉토리에 있는 vmcore 파일을 생성합니다. vmcore 파일을 확인하여 운영 중단의 원인을 파악할 수 있습니다. QLogic QLE2742 HBA 이벤트를 사용한 스토리지 페일오버는 "kmem_cache_alloc+131" 모듈에 영향을 줍니다. 커널 중단 후 "[exception RIP:kmem_cache_alloc+131]" 문자열을 찾아 vmcore 파일에서 이벤트를 찾을 수 있습니다. 호스트 OS를 재부팅하고 운영 체제를 복구합니다. 그런 다음 응용 프로그램을 다시 시작합니다. 


| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=1266250["1266250"^] | iSCSI SAN LUN에 Red Hat Enterprise Linux 8.1을 설치하는 동안 여러 경로에 로그인할 수 없습니다 | iSCSI SAN LUN 다중 경로 장치에 Red Hat Enterprise Linux 8.1을 설치하는 동안에는 여러 경로에 로그인할 수 없습니다. 다중 경로 iSCSI 장치에 설치할 수 없으며 다중 경로 서비스가 SAN 부팅 장치에서 활성화되지 않습니다. 
|===
--
.8.0
--
...
* Red Hat Linux 가상화(KVM)에 대해 알아보세요.
+
Red Hat Linux는 KVM 호스트 역할을 할 수 있습니다. 이를 통해 Linux 커널 기반 가상 머신(KVM) 기술을 사용하여 단일 물리적 서버에서 여러 가상 머신을 실행할 수 있습니다. KVM 호스트는 ONTAP LUN에 대한 명시적인 호스트 구성 설정이 필요하지 않습니다.
