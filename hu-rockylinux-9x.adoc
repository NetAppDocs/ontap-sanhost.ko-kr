...
summary: ONTAP LUN을 사용하여 FCP 및 iSCSI 프로토콜 작업을 관리하기 위해 Rocky Linux 9.x 호스트에 Linux 호스트 유틸리티를 설치합니다. 
---
= ONTAP 스토리지를 사용하여 FCP 및 iSCSI에 대해 Rocky Linux 9.x 구성
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilities 소프트웨어는 ONTAP 스토리지에 연결된 Linux 호스트에 대한 관리 및 진단 도구를 제공합니다. Rocky Linux 9.x 호스트에 Linux 호스트 유틸리티를 설치하면 호스트 유틸리티를 사용하여 ONTAP LUN을 통해 FCP 및 iSCSI 프로토콜 작업을 관리할 수 있습니다.
...
Rocky Linux는 RHEL 호환 배포판이므로 별도의 Linux Host Utilities 패키지가 없습니다. 해당 RHEL 버전에 맞는 Linux Host Utilities 패키지를 사용하십시오.
...
Rocky Linux 9.x에서 멀티패스를 사용하여 ONTAP LUN을 관리할 수 있습니다.
...
ONTAP 스토리지를 탑재한 Rocky Linux 9.x에는 다음과 같은 알려진 문제가 있습니다.
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/1508554["1508554"^] | NetApp Linux 호스트 유틸리티 CLI에서 Emulex HBA(호스트 버스 어댑터) 어댑터 검색을 지원하려면 추가 라이브러리 패키지 종속성이 필요합니다 | RHEL 9.x에서 NetApp Linux SAN Host Utilities CLI `sanlun fcp show adapter -v`는 Emulex HBA(호스트 버스 어댑터) 검색을 지원하는 라이브러리 패키지 종속성을 찾을 수 없기 때문에 실패합니다. | 해당 없음 
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/1508554["1508554"^] | NetApp Linux Host Utilities CLI에서 Emulex HBA 어댑터 검색을 지원하려면 추가 라이브러리 패키지 종속성이 필요합니다 | RHEL 9.2에서 NetApp Linux SAN Host Utilities CLI는 `sanlun fcp show adapter -v` HBA 검색을 지원하기 위한 라이브러리 패키지 종속성을 찾을 수 없기 때문에 실패합니다. 


| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/1537359["1537359)를 참조하십시오"^] | Emulex HBA를 사용하는 Red Hat Linux 9.2 SAN 부팅 호스트에서는 커널이 중단되는 작업이 발생합니다 | 스토리지 페일오버 반환 작업 중에 Emulex 호스트 버스 어댑터(HBA)가 설치된 Red Hat Linux 9.2 SAN 부팅 호스트에서 커널이 중단되는 작업이 발생합니다. 커널이 중단되면 운영 체제가 재부팅되고 `kdump` 가 구성되어 있으면 가 생성됩니다 `vmcore` 파일 아래에 있습니다 `/var/crash/` 디렉토리. 이 문제는 와 함께 삼각 측량을 진행 중입니다 `lpfc` 드라이버를 재생하지만 일관되게 재현할 수는 없습니다. 
|===
--
.9.1
--
...
* Rocky Linux 가상화(KVM)에 대해 알아보세요.
+
Rocky Linux는 KVM 호스트 역할을 할 수 있습니다. 이를 통해 Linux 커널 기반 가상 머신(KVM) 기술을 사용하여 단일 물리적 서버에서 여러 가상 머신을 실행할 수 있습니다. KVM 호스트는 ONTAP LUN에 대한 명시적인 호스트 구성 설정이 필요하지 않습니다.
